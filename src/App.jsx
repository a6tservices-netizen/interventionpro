...
function buildReportHTML(fiche, hideInternal = false) {
  ...
  const prestaCards = prestaAffichees
    .map(p => {
  ...
  const photoSection = (titre, liste, prefixe = "") => liste.length
    ? `<div class="photo-section">`
      + `<div class="photo-tete">${prefixe}<div class="photo-subtitle">${titre} (${liste.length})</div>`
  ...
  /* Le titre "Photos (N)" est place DANS le premier bloc insecable, jamais seul
     en bas de page. Le reste des photos s'ecoule, pour eviter les pages blanches. */
  const titrePhotos = `<div class="section-title">Photos (${fiche.photos?.length||0})</div>`;
  let photosCorps = "";
  if (photosOntTag) {
    const groupes = [
      ["Avant travaux", fiche.photos.filter(p=>p.tag==="avant")],
      ["Pendant intervention", fiche.photos.filter(p=>p.tag==="pendant")],
      ["Après travaux", fiche.photos.filter(p=>p.tag==="apres")],
      ["Autres photos", fiche.photos.filter(p=>!p.tag)],
    ].filter(g=>g[1].length);
    photosCorps = groupes.map((g,i)=>photoSection(g[0], g[1], i===0?titrePhotos:"")).join("");
  } else if (fiche.photos?.length) {
    photosCorps = `<div class="tete">${titrePhotos}`
      + `<div class="photo-grid">${fiche.photos.slice(0,2).map(photoItem).join("")}</div></div>`
      + (fiche.photos.length>2 ? `<div class="photo-grid">${fiche.photos.slice(2).map(photoItem).join("")}</div>` : "");
  }
  const photoGrid = fiche.photos?.length
    ? `<div class="section-block flow">${photosCorps}</div>` : "";
...
/* PAGINATION A L'IMPRESSION
   Regle unique : aucun element n'est jamais coupe entre deux pages. Un bloc qui
   ne tient pas bascule entier sur la page suivante.
   - .section-block          : rubrique courte, insecable en entier
   - .section-block.flow     : rubrique longue (compte-rendu, photos), qui peut
                               s'etaler sur plusieurs pages sans laisser de
                               demi-page blanche
   - .tete                   : titre de rubrique + son premier bloc, solidaires
                               (Chrome ignore break-after:avoid, d'ou l'enveloppe) */
@media print{
  .section-block,
  .tete,.photo-tete,
  .info-card,.presta-card,.resp-badge,.loc-banner,.preco-list li,.int-card,
  .photo-item,.sig-zone,.sig-box,.conclusion-box,.footer,
  .para,.phrase{
    break-inside:avoid;page-break-inside:avoid;
  }
  .section-block.flow{break-inside:auto;page-break-inside:auto}
  .section-title,.presta-header,.photo-subtitle{break-after:avoid;page-break-after:avoid}
  .para,.phrase,.preco-list li{orphans:3;widows:3}

  /* Mise en page resserree : plus de contenu par page, donc moins de ruptures. */
  ...
  .photo-grid{gap:8px;padding-top:2mm}
  .photo-item,.photo-item img{max-height:170px}
...
<div class="body">
  ...
  <div class="section-block flow">
    <div class="tete">
      ...
      ${prestaCards[0]||'<p style="color:#94a3b8;font-style:italic">Aucune prestation enregistrée.</p>'}
    </div>
    ${prestaCards.slice(1).join("")}
  </div>
...
function zoomPhoto(src){
  /* Le rapport est affiche dans une iframe : un overlay position:fixed se
     placerait dans le repere de l'iframe (1600px de haut), donc hors ecran sur
     mobile. On remonte le clic a l'application, qui ouvre sa vraie visionneuse.
     Si le rapport est ouvert seul (fichier telecharge), on garde le lightbox. */
  try{
    if(window.parent && window.parent !== window){
      window.parent.postMessage({type:'ip-zoom-photo', src:src}, '*');
      return;
    }
  }catch(e){}
...
function ReportPreview({ fiche, onClose, parametresMessages = {modeles:MODELES_MESSAGE_DEFAUT}, onMarquerEnvoye = null }) {
  ...
  // Clic sur une photo du rapport : l'iframe nous previent, on ouvre la
  // visionneuse plein ecran de l'application (cf. zoomPhoto dans le rapport).
  const [zoomSrc, setZoomSrc] = useState(null);
  useEffect(() => {
    const onMsg = (e) => {
      if (e.data && e.data.type === "ip-zoom-photo" && typeof e.data.src === "string") setZoomSrc(e.data.src);
    };
  ...
  return (
    <div style={{position:"fixed",inset:0,background:"#050C18",zIndex:800,display:"flex",flexDirection:"column"}}>
      ...
      {zoomSrc && (
        <PhotoViewer photos={[{data:zoomSrc}]} index={0} onClose={()=>setZoomSrc(null)} onIndexChange={()=>{}}/>
      )}
      {apercuEnvoi && (
